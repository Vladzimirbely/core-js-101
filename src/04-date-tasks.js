...
function angleBetweenClockHands(date) {
  const hours = ((date.getUTCHours() % 12) * 30) + (date.getMinutes() * 0.5);
  return (Math.min(Math.abs(date.getMinutes() * 6 - hours), 360 - Math.abs(date.getMinutes() * 6 - hours)) * Math.PI) / 180;
}
